...
function draw() {
    ...
    for (let x = 0; x < width; x++) {
        ...
        for (let y = 0; y < height; y++) {
            ...
            luminance = map(noise(xoff, yoff,speed), 0, 1, 40, 110);// max over 10 from max limit of l to make pure white "the sun behind cloud" effect
            hue = map(noise(xoff, yoff, speed), 0, 1, 192, 220);
            saturation = map(noise(xoff, yoff, speed), 0, 1, 100, 80);
